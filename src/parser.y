...
/* These define the tokens that are generated by the lexer */
...
/* These define the type of the tokens */
%type <node> page
%type <node> sentences
...
%start page /* The token from which Grammar starts */
...
sentences: 
    ...
    | sentences sentence { 
        $$ = $1;
        $$->children.push_back(unique_ptr<ASTNode>($2)); // Add each sentence as a child
    }
;
...
sentence: 
        ...
        | CONTENT   { $$ = new ASTNode("content", *$1); delete $1; }  // Handle regular content
        ;

headings: 
          SECTION CONTENT ENDBRACE { 
            $$ = new ASTNode("heading", *$2);  // Create heading node
            delete $2;
        } 
        | SUBSECTION CONTENT ENDBRACE { 
            $$ = new ASTNode("subheading", *$2);  // Create subheading node
            delete $2;
        } 
        | SUBSUBSECTION CONTENT ENDBRACE { 
            $$ = new ASTNode("subsubheading", *$2);  // Create subsubheading node
            delete $2;
        }
        ;
fonts: 
    BOLD CONTENT ENDBRACE { 
        $$ = new ASTNode("bold", *$2);  // Handle bold text
        delete $2; 
    }
    | ITALICS CONTENT ENDBRACE { 
        $$ = new ASTNode("italics", *$2);  // Handle italics text
        delete $2; 
    }
    | HORIZONTALLINE { 
        $$ = new ASTNode("horizontal_line", "---");  // Horizontal line
    }
    | PARAGRAPH { 
        $$ = new ASTNode("paragraph", "\n");  // Paragraph break
    } 
    | BOLD CONTENT ITALICS commonPart ENDBRACE {
        $$ = new ASTNode("bold_italics", " **" + *$2 + "**" + $4->value);  // Bold and italics combined
        delete $2;
        delete $4;
    }
    | BOLD CONTENT ITALICS commonPart CONTENT ENDBRACE {
        $$ = new ASTNode("bold_italics", " **" + *$2 + "**" + $4->value + " **" + *$5 + "**");  // Bold and italics with additional content
        delete $2;
        delete $4;
        delete $5;
    }
...
commonPart: 
      CONTENT ENDBRACE {
        $$ = new ASTNode("italics_and_bold", " **_" + *$1 + "_**");     // it handles both bold and italic content
        delete $1;
    }
    ;
...
code:
    CODEBLOCKSTART sametext CODEBLOCKEND {                              // handles verbatim
...
sametext:
    NEWLINE sametext {                                              // recursively handles the all the content in verbatim
        $$ = new string("\n" + *$2);
        delete $2;
    }
    | CONTENT sametext {
...
link: 
        linkfirsthalf linksecondhalf {                                     // link consist of 2 parts and it is used to handle hyperlink
...
lists: 
        UNORDEREDLIST NEWLINE unorderedListItems ENDUNORDEREDLIST {
            ...
            $$ = new ASTNode("unordered_list", "");             // used to handle unordered list
            $$->addChild(unique_ptr<ASTNode>($3));
        }
        | ORDEREDLIST NEWLINE orderedListItems ENDORDEREDLIST {
            countOrderNo = 0;
            indentLevel = -4;                                   // Reset indent level
            $$ = new ASTNode("ordered_list", "");               // Handle ordered list
            $$->addChild(unique_ptr<ASTNode>($3));
        }
        ;

unorderedListItems:                                                    // used to handle nested unordered list
...
orderedListItems:                                           // used to handle nested ordered list
...
ignore: // to ignore all these characters
